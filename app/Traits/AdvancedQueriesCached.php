...
trait AdvancedQueriesCached
{
    ...
    /**
     * Gera uma chave única para o cache com base nos parâmetros da consulta.
     *
     * @param array $attr
     * @param array $options
     * @return string
     ...
     * @param array $attr
     * @param array $options
     * @param int $cacheTTL Tempo de vida do cache em segundos (opcional).
     * @return Collection|LengthAwarePaginator
     * @throws Exception
     */
    public function advancedQuery(Builder $query, array $attr, array $options, int $cacheTTL = 3600): Collection | LengthAwarePaginator
    {
        $cacheKey = $this->generateCacheKey($attr, $options);
        ...
        if (array_key_exists('paginate', $attr) && ! empty($attr['paginate']) && is_numeric($attr['paginate'])) {
            $result = $query->paginate(intval($attr['paginate']));
        } else {
            $result = $query->get();
        }
